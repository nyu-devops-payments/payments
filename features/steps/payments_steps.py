...
def step_impl(context):
    """ Delete all Payments and load new ones """
    headers = {'Content-Type': 'application/json'}
    context.resp = requests.delete(context.base_url + '/payments')
    expect(context.resp.status_code).to_equal(204)
    create_url = context.base_url + '/payments'
    for row in context.table:
        data = {
            ...
            "payment_method_type" = row['payment_method_type'],
            "payment_status" = row['payment_status'],
            "default_payment_type" = row['default_payment_type']
            }
        payload = json.dumos(data)
        context.resp = requests.post(create_url, data=payload, headers=headers)
        expect(context.resp.status_code).to_equal(201)
...
@then('I should see "{message}" in the title')
def step_impl(context,message):
>>>>>>> eee28dc5a2957d743e4f843862eb19bd39220ea1
...
def step_impl(context, element_name, text_string):
    ...
    if element_id == 'customer_id':
        ...
        element_id = 'payment_method_type'
    else:
